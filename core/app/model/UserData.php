...
class UserData
{
	...
	public function add()
	{
		// Usar NOW() directamente en la consulta
		$sql = "INSERT INTO user (name, lastname, username, email, is_admin, password, created_at) ";
		$sql .= "VALUES (\"$this->name\", \"$this->lastname\", \"$this->username\", \"$this->email\", \"$this->is_admin\", \"$this->password\", NOW())"; // Cambiar $this->created_at por NOW()
		Executor::doit($sql);
	}
